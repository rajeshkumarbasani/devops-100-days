...
This Terraform module provisions an AWS EC2 instance and a supporting security group for Day 6.
It is designed to be consumed by the environment configuration in [`terraform/day6/env/dev/main.tf`](../env/dev/main.tf).
...
- `aws_instance.this`
  ...
  - Enables monitoring and enforces IMDSv2 metadata access.
...
This module is consumed by [`terraform/day6/env/dev/main.tf`](../env/dev/main.tf):
...
The `ec2` module depends on outputs from the VPC module, so the `vpc` module must be applied first.

## Module integration

- This module is linked from [`../env/dev/main.tf`](../env/dev/main.tf).
- It consumes network outputs from [`../vpc/README.md`](../vpc/README.md).

## Why this module exists

The EC2 module encapsulates compute provisioning and security configuration so that:

- network and compute responsibilities remain separated
